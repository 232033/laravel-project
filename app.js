...
async function loadProducts(){
  ...
  // 1) Firestore realtime (if configured)
  try {
    if (window.db) {
      if (!window.__trixa_subscribed) {
        window.__trixa_subscribed = true;
        window.__trixa_unsub = window.db.collection('products').onSnapshot(
          (snap)=>{
            __liveItems = snap.docs.map(d=>{
              const data = d.data()||{};
              return { docId: d.id, ...data, id: (data.id ?? d.id) };
            });
            renderProducts(applyFilter(__liveItems));
          },
          (err)=>console.warn('Firestore onSnapshot error', err)
        );
      }
      // If we have live items already, render them; otherwise a first render will happen on the snapshot callback
      if(__liveItems.length){ renderProducts(applyFilter(__liveItems)); }
      return;
    }
  } catch(e) { /* ignore and fallback */ }

  // 2) Netlify Function (if available)
  let items = [];
  try {
    const res = await fetch('/.netlify/functions/products');
    if(res.ok){ items = await res.json(); }
  } catch(e){ /* likely running locally via file:// or no functions available */ }
...
function renderCard(p){
  ...
  <article class="card" aria-label="${p.title}">
    <a id="link-img-${key}" class="card__img" href="product.html?${q.toString()}" data-link data-id="${p.docId||p.id}">
    ...
    <div class="card__body">
      <a id="link-title-${key}" class="card__title" href="product.html?${q.toString()}" data-link data-id="${p.docId||p.id}">${p.title}</a>
      <div class="price">
...
async function loadSettingsFront(){
  ...
  try{
    if(window.db){
      ...
      __settings = doc.exists ? doc.data() : {};
    }
  }catch(_){ /* ignore */ }
  if(!__settings) __settings = {};
  if(!__settings.waNumber){ __settings.waNumber = localStorage.getItem('WA_NUMBER') || ''; }
...
// Banners: subscribe and render if Firestore available
async function subscribeBanners(){
  if(!(window.db && els.sliderTrack)) return;
  try{
    window.db.collection('banners').orderBy('createdAt','desc').onSnapshot((snap)=>{
      __banners = snap.docs.map(d=>({ id:d.id, ...d.data() }));
      renderBanners(__banners);
  ...
  els.sliderTrack.innerHTML = items.map(b=>`
    <article class="slide" style="--bg:${b.bg||'#3f2b82'};background:${b.bg||'#3f2b82'}">
      <div class="slide__content">
        <h2>${b.title||''}</h2>
        ${b.text?`<p>${b.text}</p>`:''}
        ${b.ctaText?`<a href="${b.ctaLink||'#'}" class="btn btn-light">${b.ctaText}</a>`:''}
      </div>
